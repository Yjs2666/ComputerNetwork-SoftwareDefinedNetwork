...
```
For this assignment, you are not required to handle all fields in this stcp_header. Specifically, the provided network layer wrapper code sets th_sport, th_dport, and th_sum, while th_urp is unused; you may thus ignore these fields. Similarly, you are not required to handle all legal flags specified here: TH_RST, TH_PUSH, and TH_URG are ignored by STCP. The fields STCP uses are shown in the following table. Note that any relevant multi-byte fields of the STCP stcp_header will entail proper endianness handling with `htonl`/`ntohl` or `htons`/`ntohs`

The packet stcp_header field format (for the relevant fields) is as follows: 
...
- The `th_seq` field in the packet stcp_header contains the sequence number of the first byte in the payload.
...
1. The requesting active end sends a SYN packet to the other end with the appropriate seq number (seqx) in the stcp_header. The active end then sits waiting for a SYN_ACK.
