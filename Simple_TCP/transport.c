...
{
    CSTATE_ESTABLISHED,
    FIN_WAIT_1,         /*the first ending side waiting for FIN-ACK*/
    FIN_WAIT_2,         /* the first ending side waitign for FIn from the other side*/
    CLOSE_WAIT_PASSIVE, /*when one side ends connection*/
    TIME_WAIT,          /*the second ending side waiting for FIN-ACK*/
    CSTATE_CLOSED
};
...
static void buffer_control_APP_DATA(mysocket_t sd, context_t *ctx)
{
    ...
    ssize_t nk_pkt_size = stcp_app_recv(sd, data_buffer, MIN(ctx->received_window_size - consumed_window, STCP_MSS));

    //////////////
    // send_data_packet(sd, ctx, 0, FALSE, data_buffer, nk_pkt_size);
    ...
    free(packet);
    ctx->current_sequence_num += (nk_pkt_size > 0) ? nk_pkt_size : 1;
    //////////////
    free(data_buffer);
}