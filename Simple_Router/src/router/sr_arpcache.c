...
void handle_arpreq(struct sr_instance *sr, struct sr_arpreq *request) {
    ...
    #define ARP_REQ_MAX 5

    if (difftime(now, request->sent) > 1.0) {
        
        if (request->times_sent >= ARP_REQ_MAX) {
            struct sr_packet *packet_t = request->packets;
            /* Send ICMP Host Unreachable to source address of all packets waiting on this request */
            printf("Resent 5 times alrealy, Send ICMP Host Unreachable.\n");
            fprintf(stderr, "Resent 5 times alrealy, Send ICMP Host Unreachable.\n");
            struct sr_if *iface = sr_get_interface(sr, packet_t->iface);
            send_icmp_message(sr, packet_t->buf, iface, 3, 1, packet_t->len);

            // while (packet_t) {
            //     struct sr_if *iface = sr_get_interface(sr, packet_t->iface);
            //     send_icmp_message(sr, packet_t->buf, iface, 3, 1, packet_t->len);
            //     packet_t = packet_t->next;
            // }
