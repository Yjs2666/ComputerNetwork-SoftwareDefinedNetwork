...
void forward_ip_l2(struct sr_instance* sr, sr_ip_hdr_t* ip_hdr, sr_ethernet_hdr_t* eth_hdr, uint8_t* packet, unsigned int len, struct sr_if* if_curr) {
    ...
    while (entry != NULL) {
      // 使用掩码与目的地址进行比较，找到最长匹配
      if ((dest_ip & entry->mask.s_addr) == (entry->dest.s_addr & entry->mask.s_addr)) {
        // 比较找到的掩码是否比当前记录的更长
        if (entry->mask.s_addr > max_mask) {
          max_mask = entry->mask.s_addr;
          matched_entry = entry;
        }
      }
      entry = entry->next;
    ...
    if (arp_entry) {
        ...
        sr_arpcache_queuereq(&(sr->cache), matched_entry->gw.s_addr, packet, len, out_if->name);
        // sr_arpcache_queuereq(&(sr->cache), matched_entry->gw.s_addr, packet, len, out_if->name);
    }
}
