...
def setupQoS(net):
    qosCmd1 = 'sudo ovs-vsctl -- --all destroy QoS -- --all destroy Queue'
    qosCmd2 = '''sudo ovs-vsctl -- set Port s1-eth1 qos=@newqos -- --id=@newqos create QoS type=linux-htb other-config:max-rate=10000000 queues:1=@q1,2=@q2 -- --id=@q1 create Queue other-config:min-rate=8000000 other-config:max-rate=10000000 -- --id=@q2 create Queue other-config:min-rate=5000000 other-config:max-rate=5000000'''
    os.system(qosCmd1)
    os.system(qosCmd2)
...
def startNetwork():
    ...
    net.addController('c0', controller=RemoteController, ip=controllerIP, port=6633)
